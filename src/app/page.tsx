...
import { CHALLENGES, formatTime, type SquadSize } from "@/game/types";
...
function GameplayWorld() {
  return (
    <div className="world-frame" aria-label="A stylized view of the Many Hands physics world" role="img">
      ...
      <div className="world-frame__hud world-frame__hud--bottom"><span>ALL HANDS LIVE</span><span className="world-frame__meter"><span /></span><span>72%</span></div>
      <div className="world-frame__caption">Everybody drives. Nobody promises to drive well.</div>
    </div>
  );
}
    ...
    <main className="landing-page">
      ...
      <div className="landing-shell">
        ...
        <section id="top" className="hero-grid">
          <div className="hero-copy"><p className="eyebrow"><span className="eyebrow__line" /> Co-op physics party game</p><h1>Everybody drives.<br /><em>Nobody behaves.</em></h1><p className="hero-description">Every player waddles and steers. Grab with either hand, hop, boost, yeet, and quack your way through the course together.</p><div className="hero-specs" aria-label="Game features"><span><strong>3 / 5</strong> players</span><span><strong>ALL</strong> driving</span><span><strong>∞</strong> quacks</span></div></div>
          <div className="hero-visual"><GameplayWorld /></div>
          <section className="action-panel" aria-labelledby="start-heading"><div className="action-panel__header"><div><p className="eyebrow eyebrow--small">Get into the world</p><h2 id="start-heading">Start a session</h2></div><span className="action-panel__index">02</span></div><label className="field-label" htmlFor="player-name">Your name</label><input id="player-name" value={name} onChange={(e) => setName(e.target.value)} maxLength={16} autoComplete="nickname" placeholder="e.g. Captain Quack" className="field-input" /><p className="field-hint">16 characters max. A name is generated if you leave this blank.</p><div className="action-grid"><button type="button" disabled={busy} onClick={() => create(false)} className="button button--primary"><span>Create room</span><small>3 or 5 friends, all driving</small><span className="button__arrow" aria-hidden="true">↗</span></button><button type="button" disabled={busy} onClick={() => create(true)} className="button button--secondary"><span>Solo practice</span><small>Learn the waddle and the chaos</small><span className="button__arrow" aria-hidden="true">↗</span></button></div><div className="join-divider"><span>or join an existing room</span></div><form className="join-form" onSubmit={(event) => { event.preventDefault(); join(); }}><label className="sr-only" htmlFor="room-code">Four-character room code</label><input id="room-code" value={code} onChange={(e) => { setCode(e.target.value.toUpperCase().replace(/[^A-HJ-NP-Z2-9]/g, "")); setJoinError(""); }} maxLength={4} inputMode="text" autoComplete="off" placeholder="ROOM CODE" aria-invalid={Boolean(joinError)} aria-describedby={joinError ? "room-code-error" : "room-code-hint"} className="field-input field-input--code" /><button type="submit" disabled={busy} className="button button--join">Join <span aria-hidden="true">↗</span></button></form>{joinError ? <p id="room-code-error" className="form-message form-message--error" role="alert">{joinError}</p> : <p id="room-code-hint" className="field-hint">Ask your host for the code displayed in their lobby.</p>}{busy && <p className="form-message form-message--loading" role="status"><span className="loading-bar" /> Preparing your physics world...</p>}</section>
        </section>
        <section className="roles-section" aria-labelledby="roles-heading"><div className="section-heading"><div><p className="eyebrow eyebrow--small">One moveset for everyone</p><h2 id="roles-heading">Waddle. Grab. Yeet. Quack.</h2></div><p>No assigned limbs and no waiting for your turn. Every player gets the full gloriously unstable moveset.</p></div><div className="roles-grid"><div className="role-card"><span className="role-card__number">EVERYONE DRIVES</span><span className="control-card__keys" aria-label="W A S D"><kbd>W</kbd><kbd>A</kbd><kbd>S</kbd><kbd>D</kbd></span><span className="role-card__label">Waddle + steer</span><span className="role-card__blurb">Every direction is a group decision. Good luck with that.</span></div><div className="role-card"><span className="role-card__number">LEFT HAND</span><span className="control-card__keys" aria-label="Q plus W A S D"><kbd>Q</kbd><span aria-hidden="true">+</span><kbd>WASD</kbd></span><span className="role-card__label">Reach + grab</span><span className="role-card__blurb">Hold Q, then move to grab, drag, and flail with the left hand.</span></div><div className="role-card"><span className="role-card__number">RIGHT HAND</span><span className="control-card__keys" aria-label="E plus W A S D"><kbd>E</kbd><span aria-hidden="true">+</span><kbd>WASD</kbd></span><span className="role-card__label">More grabby trouble</span><span className="role-card__blurb">Hold E, then move to take the right hand. Two crewmates make a proper two-hand grab.</span></div><div className="role-card"><span className="role-card__number">WHOLE CREW</span><span className="control-card__keys"><kbd>SPACE</kbd></span><span className="role-card__label">Hop</span><span className="role-card__blurb">Pop everyone upward. Timing is optional; landing is not.</span></div><div className="role-card"><span className="role-card__number">CHAOS BUTTON</span><span className="control-card__keys"><kbd>SHIFT</kbd></span><span className="role-card__label">Duck-boost + yeet + quack</span><span className="role-card__blurb">Duck under trouble, add speed, launch your cargo together, and announce it loudly.</span></div></div></section>
